...
def broadcast_up(alpha, t, x_shape):
  import ipdb; ipdb.set_trace()
  bs, = t.shape
  assert x_shape[0] == bs
  out = tf.gather(tf.convert_to_tensor(alphas, dtype=tf.float32), t)
  assert out.shape == [bs]
...
class GaussianDiffuser:
  ...
  def q_mean_variance(self, x_start, t):
    import ipdb; ipdb.set_trace()
    mean = broadcast_up(self.sqrt_alphas_cumprod, t, x_start) * x_start
    variance = broadcast_up(1. - self.alphas_cumprod, t, x_start)
    log_variance = broadcast_up(self.log_one_minus_alphas_cumprod, x_start)
  ...
  def q_sample(self, x_start, t, noise=None):
    ...
    return (
        broadcast_up(self.sqrt_alphas_cumprod, t, x_start) * x_start +
        broadcast_up(self.sqrt_one_minus_alphas_cumprod, t, x_start) * noise
    )
  ...
  def q_posterior_mean_variance(self, x_start, x_t, t):
    ...
    posterior_mean = (
        broadcast_up(self.posterior_mean_coef1, t, x_t) * x_start +
        broadcast_up(self.posterior_mean_coef2, t, x_t) * x_t
    )
    posterior_variance = broadcast_up(self.posterior_variance, t, x_t)
    posterior_log_variance_clipped = broadcast_up(self.posterior_log_variance_clipped, t, x_t)
  ...
  def p_mean_variance(self, denoise_fn, *, x, t, clip_denoised: bool, return_pred_xstart: bool):
    ...
    if self.model_var_type == 'learned':
      ...
      model_variance = broadcast_up(model_variance, t, x) * torch.ones(x.shape.as_list())
      model_log_variance = broadcast_up(model_log_variance, t, x) * torch.ones(x.shape.as_list())
    else:
      raise NotImplementedError(self.model_var_type)

    # Mean parameterization
    def _maybe_clip(x_): return torch.clamp(x_, -1, 1) if clip_denoised else x_
  ...
  def _predict_xstart_from_eps(self, x_t, t, eps):
    ...
    return (
        broadcast_up(self.sqrt_recip_alphas_cumprod, t, x_t) * x_t -
        broadcast_up(self.sqrt_recipm1_alphas_cumprod, t, x_t) * eps
    )

  def _predict_xstart_from_xprev(self, x_t, t, xprev):
    assert x_t.shape == xprev.shape
    return (  # (xprev - coef2*x_t) / coef1
        broadcast_up(1. / self.posterior_mean_coef1, t, x_t) * xprev -
        broadcast_up(self.posterior_mean_coef2 / self.posterior_mean_coef1, t, x_t) * x_t
    )
